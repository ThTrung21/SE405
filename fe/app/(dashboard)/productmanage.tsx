...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, Modal, TextInput, Alert, ScrollView, Image, Picker } from 'react-native';
...
export default function ProductManage() {
  ...
  const [brandItems, setBrandItems] = useState(brands.map(b => ({ label: b, value: b })).concat([{ label: '+ Add new brand', value: '__add_new__' }]));
      ...
      <View style={styles.categoryRow}>
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              <Ionicons name={item.icon} size={20} color={selectedCategory === item.key ? '#fff' : '#222'} />
              <Text style={[styles.categoryLabel, selectedCategory === item.key && { color: '#fff' }]}>{item.label}</Text>
            </TouchableOpacity>
          )}
        />
      </View>
